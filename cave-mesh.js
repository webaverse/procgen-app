...
export class CaveMesh extends BatchedMesh {
  constructor({
    instance,
    gpuTaskManager,
  }) {
    ...
    const task = this.gpuTaskManager.transact(() => {
      ...
      const _renderWaterMeshDataToGeometry = (
        ...
        geometry.attributes.position.update(
          positionOffset,
          caveGeometry.positions.length,
          caveGeometry.positions,
          0
        );
        geometry.attributes.normal.update(
          normalOffset,
          caveGeometry.normals.length,
          caveGeometry.normals,
          0
        );
        geometry.index.update(indexOffset, caveGeometry.indices.length);
      };
      const _handleCaveMesh = caveGeometry => {
      ...
      _handleCaveMesh(chunkResult.caveGeometry);
