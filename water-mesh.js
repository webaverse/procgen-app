...
export class WaterMesh extends BufferedMesh {
  ...
  removeChunk(chunk) {
    ...
    {
      const key = chunk.min.x + ',' + chunk.min.y;
      const chunkPhysicObjec = this.currentChunkMap.get(key);
      const waterHeightObject = this.currentWaterHeightMap.get(key);
      if (chunkPhysicObjec) {
        this.currentChunkMap.delete(key)
      }
      if (waterHeightObject) {
        this.currentWaterHeightMap.delete(key)
      }
    }
    {
