...
// import {getTempRenderer} from '../utils/renderer-utils.js';

export class ImmediateGLBufferAttribute extends THREE.GLBufferAttribute {
  constructor(array, itemSize, ctx, isIndex = false) {
    if (!ctx?.useRenderer) {
    ...
    const renderer = ctx.useRenderer();
    // const renderer = getTempRenderer();
    // const renderer = null;
    // console.warn('TODO: renderer is null, need to pass it in');
    // debugger;
  ...
  wrap(fn, renderer) {
    ...
    if (!ImmediateGLBufferAttribute.pushed) {
      popUpdate = ImmediateGLBufferAttribute.pushUpdate(renderer);
    }
  ...
  // update(offset, count, array = this.array, srcOffset = offset, renderer) {
  update(offset, count, array, srcOffset, renderer) {
    if (
      offset === undefined ||
      count === undefined ||
      array === undefined ||
      ...
      console.warn('missing args', {offset, count, array, srcOffset, renderer});
      debugger;
    }
