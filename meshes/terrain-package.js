...
class TerrainPackage {
  ...
  static async loadUrls(paths, appCtx) {
    if (!appCtx) {
      console.warn('missing appCtx', {paths, appCtx});
      debugger;
    }

    const loaders = appCtx.useLoaders();
    const {exrLoader} = loaders;
    // const {CanvasTextureAtlas} = this.ctx.useAtlasing();
